module dshr_strdata_mod
  ...
  use ESMF             , only : ESMF_DistGrid, ESMF_MeshGet, ESMF_DistGridGet
...
!===============================================================================
  ...
  subroutine shr_strdata_init(sdat, model_clock, stream_name, rc)
    ...
    loop_over_streams1: do ns = 1,shr_strdata_get_stream_count(sdat)
       ...
       if (sdat%stream(ns)%readmode=='single') then
          ...
          if (mainproc) then
             write(logout,'(2a,i0)') trim(subname), &
                  " Creating field bundle array fldbun_data of size 2 for stream ",ns
          end if
       ...
       do nfld = 1, nvars
          do i=1,size(sdat%pstrm(ns)%fldbun_data)
             ...
             if (mainproc) then
                if (i == 1) then
                   write(logout,'(4a,i0)') trim(subname),&
    ...
    loop_over_stream2: do ns = 1,shr_strdata_get_stream_count(sdat)
       ...
       if (trim(stream_vector_names) /= 'null') then
          ...
          if (chkerr(rc,__LINE__,u_FILE_u)) return
          if (mainproc) then
             write(logout,'(4a,i0)') trim(subname)," creating ESMF stream vector field with names", &
    ...
    if (mainproc) then
       ...
       write(logout,'(2a)') trim(subname),' successfully initialized sdat'
    endif
  ...
  subroutine shr_strdata_get_stream_nlev(sdat, stream_index, rc)
    ...
    if (mainproc) then
       write(logout,*)
       write(logout,'(2a,i0)') trim(subname),' stream_nlev = ',stream_nlev
       if (stream_nlev /= 1) then
  ...
  subroutine shr_strdata_advance(sdat, ymd, tod, logunit, istr, timers, rc)
    ...
    if(present(timers)) then
       write(logout,'(2a)') trim(subname),'optional variable timers present but unused'
    endif
    ...
    if (nstreams > 0) then
       ...
       do ns = 1,nstreams
          ...
          case default
             write(logout,'(2a)') "ERROR: Unsupported readmode : ", trim(sdat%stream(ns)%readmode)
             rc = ESMF_FAILURE
          ...
          if (newData(ns)) then
             if (trim(sdat%model_calendar) /= trim(sdat%stream(ns)%calendar)) then
                if ((trim(sdat%model_calendar) == trim(shr_cal_noleap)) .and. &
                  ...
                  (trim(sdat%stream(ns)%calendar) == trim(shr_cal_noleap))) then
                   ! case (3), abort
                   write(logout,'(3a)') trim(subname),' ERROR: mismatch calendar ', &
       ...
       do ns = 1,nstreams
          if (trim(sdat%stream(ns)%tinterpalgo) == 'coszen') then
             ...
             if (debug > 0 .and. mainproc) then
                do n = 1,size(coszen)
                   write(logout,'(2a,4(i0,2x),es13.6)') trim(subname),&
  ...
  subroutine shr_strdata_print(sdat, name)
    ...
    do ns = 1, shr_strdata_get_stream_count(sdat)
       ...
       write(logout,'(2a,i0,2a)')       trim(subname),"  readmode(",ns,") = ",trim(sdat%stream(ns)%readmode)
       write(logout,'(2a,i0,2a)')       trim(subname),"  vectors (",ns,") = ",trim(sdat%stream(ns)%stream_vectors)
       write(logout,'(2a,i0,a,i0)')     trim(subname),"  src_mask(",ns,") = ",sdat%stream(ns)%src_mask_val
       write(logout,'(2a,i0,a,i0)')     trim(subname),"  dst_mask(",ns,") = ",sdat%stream(ns)%dst_mask_val
  ...
  subroutine shr_strdata_readstrm(sdat, per_stream, stream, fldbun_data, &
    ...
    if (mainproc) then
       write(logout,'(5a)') trim(subname),' reading file ',trim(boundstr),': ',trim(filename)
    endif
    ...
    do nf = 1,size(per_stream%fldlist_stream)
       ...
       if (debug>0 .and. mainproc)  then
          write(logout,'(6a,i0)') trim(subname),&
          ...
          if (stream_nlev > 1) then
             ...
             if (handlefill) then
                ...
                if (stream%mapalgo == 'none' .and. any(data_dbl1d == fillvalue_r8)) then
                   write(errmsg,*) ' ERROR: _Fillvalue found in stream input variable: '// trim(per_stream%fldlist_stream(nf))
                   call shr_log_error(trim(subname)//trim(errmsg), rc=rc)
                   return
                endif
                do n = 1,size(dataptr1d)
  ...
  subroutine shr_strdata_set_stream_iodesc(sdat, per_stream, fldname, pioid, rc)
    ...
    if ( istat /= 0 ) then
       call shr_log_error(subName//'allocation error of dimids with size '// &
            toString(ndims), rc=rc)
       return
    end if

    allocate(dimlens(ndims), stat=istat)
    if ( istat /= 0 ) then
       call shr_log_error(subName//'allocation error of dimlens with size '// &
            toString(ndims), rc=rc)
       return
    end if
    ...
    if ( istat /= 0 ) then
       call shr_log_error(subName//'allocation error of compdof '// &
            toString(lsize), rc=rc)
       return
    end if
    ...
    !-------------------------------
       ...
       if ((trim(dimname) == 'time' .or. trim(dimname) == 'nt')) then
          if (mainproc) then
             write(logout,'(4a,i0,a)') trim(subname),' setting iodesc for 2d: ',trim(fldname), &
          ...
          if (mainproc) then
             write(logout,'(4a,i0,2x,i0,a)') trim(subname),' setting iodesc for 2d: ',trim(fldname), &
    ...
    !-------------------------------
       ...
       if (trim(dimname) == 'time' .or. trim(dimname) == 'nt') then
          if (stream_nlev > 1) then
             if (mainproc) then
                write(logout,'(4a,i0,2x,i0,a)') trim(subname), &
          ...
          if (stream_nlev > 1) then
             if (mainproc) then
                write(logout,'(4a,3(i0,2x),a)') trim(subname), &
